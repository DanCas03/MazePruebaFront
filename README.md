...
- **Board geometry seam (front#126)**: `presentation/game/geometry/` introduces `BoardGeometry`, an abstraction over how a board is sized and its cells projected to screen space, with two implementations — `RectGeometry` (byte-identical to the previous inline square-grid math) and `HexGeometry` (flat-top hexagonal layout, over `HexSpace`/Direction 8 from front#124-125). `BoardView` picks the geometry from the mounted `BoardSpace` and threads it through the same painters instead of branching per shape: the rect path is unchanged, and the hex path paints through `HexBoardSurfacePainter`, sharing hit-testing, arrow rendering, and the snake-exit animation with the campaign board. The hex mode-selection screen (route `/hex`) and served hex levels arrive with front#127 and back#60 — this fragment is render-only, verified by tests, with visual sign-off deferred to #127.
