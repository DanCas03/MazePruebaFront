...
- **Silueta temática (front#88, cierra el punto B)**: al montar un nivel **temático** (`palette != null`), `GameController` realiza el tablero sobre su **silueta** — `ArrowBoard.withSilhouetteSpace()` sustituye el espacio por un `MaskedSpace` cuya máscara es la **unión de las celdas de las flechas iniciales**. Combinado con el render consciente del espacio, el tablero temático se pinta como su figura (no como la caja `cols×rows`) y los toques fuera de la figura se rechazan; la campaña (rect) queda intacta. La derivación no reduce la resolubilidad (el carril de salida enmascarado es prefijo del rectangular) y es puramente en memoria (sin tocar el wire; máscaras transportadas → Fase 2/ADR-0004).
