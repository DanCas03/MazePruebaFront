...
void main() {
  ...
  test('delegates to repo.register with valid email + password and returns Right', () async {
    // Arrange
    final token = AuthToken('header.payload.sig');
    when(repo.register(any, any)).thenAnswer((_) async => Right(token));
    // Act
    final result = await useCase.execute('new@arrowmaze.com', 'sup3rs3cret');
    // Assert
    expect(result, Right<AuthFailure, AuthToken>(token));
    final captured = verify(repo.register(captureAny, captureAny)).captured;
  ...
  test('forwards EmailAlreadyRegistered from repo', () async {
    ...
    // Act
    final result = await useCase.execute('taken@arrowmaze.com', 'sup3rs3cret');
    // Assert
  ...
  test('returns Left without hitting repo when password shorter than 8', () async {
    // Act
    final result = await useCase.execute('new@arrowmaze.com', 'short');
    // Assert
